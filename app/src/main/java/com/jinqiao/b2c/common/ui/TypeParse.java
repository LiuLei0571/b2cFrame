package com.jinqiao.b2c.common.ui;
...
import com.jinqiao.b2c.compent.base.SimpleTextWatcher;
...
public class TypeParse {
    ...
    public static void setMaskType(EditText editText, String type) {
        if (TextUtils.isEmpty(type)) return;
        switch (type) {
            case "telephone":
                editText.addTextChangedListener(new SimpleTextWatcher() {
                    ...
                    public void onTextChanged(CharSequence s, int start, int before, int count) {
                        Util.setEmptyText(editText, s, count, 3, 7, 11);
                    }
    ...
    public static void setUserAction(Map<String, String> map, EditText value) {
        ...
        for (String key : keys) {
            if (map.containsKey(key)) {
                switch (key) {
                    case "maskType":
                        setMaskType(value, map.get(key));
                        break;
                    case "maxLength":
                        setMaxLength(value, Integer.valueOf(map.get(key)));
                        break;
                    case "valueType":
                        setInputType(value, map.get(key));
                        break;
                }
            }
        }
